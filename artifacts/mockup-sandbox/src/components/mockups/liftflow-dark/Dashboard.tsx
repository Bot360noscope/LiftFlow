...
const Ring = ({ percent, color, size = 100, sw = 8, label = '', subLabel = '' }: { percent: number; color: string; size?: number; sw?: number; label?: string; subLabel?: string }) => {
  ...
  return (
    <div style={{ position: 'relative', width: size, height: size, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <div style={{ zIndex: 1, textAlign: 'center' }}>
        {label && <div style={{ color: '#fff', fontWeight: 700, fontSize: size * 0.22, lineHeight: 1 }}>{label}</div>}
        {subLabel && <div style={{ color: '#888', fontSize: 9, marginTop: 2 }}>{subLabel}</div>}
      </div>
    </div>
  );
};
...
export default function Dashboard() {
  const clients = [
    { initial: 'J', name: 'Jordan M.', meta: '2 programs', status: SUCCESS, pct: 84 },
    { initial: 'C', name: 'Casey R.', meta: '1 program', status: WARNING, pct: 52 },
    { initial: 'T', name: 'Taylor B.', meta: '1 program', status: '#FF3B30', pct: 23 },
    { initial: 'M', name: 'Morgan K.', meta: '2 programs', status: SUCCESS, pct: 91 },
  ];
    ...
    <div style={{ width: 390, minHeight: '100vh', background: BG, color: T.text, fontFamily: 'system-ui,-apple-system,sans-serif', overflowY: 'auto', paddingBottom: 80 }}>
      ...
      <div style={{ padding: '48px 20px 16px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
          ...
          {/* LiftFlow badge + plan — home page only */}
      ...
      <div style={{ padding: '0 16px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
        <div style={{ ...card, padding: 16, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          <Ring percent={68} color={PRIMARY} size={96} sw={8} label="68%" subLabel="adherence" />
          <p style={{ color: T.textMuted, fontSize: 11, margin: '10px 0 0', textAlign: 'center' }}>Weekly Adherence</p>
        </div>
        <div style={{ ...card, padding: 16, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          <Ring percent={(9 / 12) * 100} color={SUCCESS} size={96} sw={8} label="9/12" subLabel="clients" />
      ...
      <div style={{ padding: '0 16px', marginBottom: 20 }}>
        <div style={{ ...card, padding: 16, border: `1px solid ${WARNING}33` }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
            ...
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={T.textDim} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m9 18 6-6-6-6"/></svg>
          </div>
          {[
            ...
            <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingTop: i > 0 ? 10 : 0, borderTop: i > 0 ? `1px solid ${T.cardBorder}` : 'none' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <div style={{ width: 30, height: 30, borderRadius: 15, background: `${PRIMARY}22`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700, color: PRIMARY }}>{r.name[0]}</div>
                <div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <div style={{ width: 26, height: 26, borderRadius: 7, background: `${WARNING}22`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: WARNING }}>
          ...
          <div style={{ marginTop: 12, paddingTop: 10, borderTop: `1px solid ${T.cardBorder}`, textAlign: 'center' }}>
      ...
      {/* Clients */}
      <div style={{ padding: '0 16px', marginBottom: 20 }}>
        <h2 style={{ fontSize: 17, fontWeight: 700, margin: '0 0 12px', color: T.text }}>Clients</h2>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {clients.map((c) => (
            <div key={c.name} style={{ ...card, padding: '12px 14px', display: 'flex', alignItems: 'center', gap: 12 }}>
              <div style={{ width: 40, height: 40, borderRadius: 20, border: `2px solid ${c.status}`, display: 'flex', alignItems: 'center', justifyContent: 'center', background: `${c.status}12`, fontSize: 16, fontWeight: 700, color: c.status, flexShrink: 0 }}>{c.initial}</div>
              <div style={{ flex: 1 }}>
              ...
              <span style={{ color: T.textMuted, fontSize: 12, fontWeight: 600, flexShrink: 0 }}>{c.pct}%</span>
            </div>
          ))}
        </div>
      </div>
